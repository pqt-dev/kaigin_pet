...
class _GoalsView extends StatelessWidget {
  ...
  Widget _buildSection(
      BuildContext context, String title, List<Goal> goals) {
          ...
          children: [
            ...
            ...goals.map(
              (goal) => GoalItemCard(
                goal: goal,
                onTap: () async {
                  final xp = await context.read<GoalsCubit>().completeGoal(goal.id);
  ...
  void _showXpSnackBar(BuildContext context, int xp) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(LocaleKeys.pet_xp_earned.tr(namedArgs: {'xp': '$xp'})),
