...
## State Management
* **BLoC Pattern:** Use `flutter_bloc` for all state management. Prefer `Cubit` for simple
  method-driven state; use `Bloc` (event-driven) only when async event transformation,
  debouncing, or complex event-to-state mapping is needed.
* **Lifecycle:** Global/shared state → `@lazySingleton`. Screen-scoped state → `@injectable`
  (factory, lifecycle owned by `BlocProvider`).
* **No async in constructor:** Never call async operations inside a BLoC/Cubit constructor.
  Trigger initialization via `BlocProvider(create: (_) => getIt<MyCubit>()..init())`.
* **State types:** Cubit state should use domain types, not Flutter framework types
  (e.g., emit `AppThemeMode`, not `ThemeMode`). Convert to framework types at the widget boundary.
* **BlocObserver:** Register a global `AppBlocObserver` in `main()` (debug mode only) to log
  all state transitions and errors.

## Dependency Injection
* **injectable + get_it:** Use `injectable` annotations and `get_it` for DI.
  Run `flutter pub run build_runner build` after changing annotations.
* **Lifecycle rules:**
  - `@lazySingleton` — global/shared app state (theme, favourites, auth)
  - `@injectable` (factory) — screen-scoped state managed by `BlocProvider`
  - `@singleton` — services that must initialize eagerly (e.g., `InternetConnectionService`)
* **Modules:** Group third-party registrations in `@module` abstract classes
  (e.g., `NetworkModule`, `StorageModule`).
